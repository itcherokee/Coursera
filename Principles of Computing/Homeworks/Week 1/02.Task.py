# http://www.codeskulptor.org/#user34_QhmbdapUbm_4.py
...
def resources_vs_time(upgrade_cost_increment, num_upgrade):
    ...
    for upgrade in range(num_upgrade):
        next_update_time = current_cost / current_rate
        total_resources_generated += next_update_time * current_rate
        current_rate +=1
        current_cost += upgrade_cost_increment
        current_time += next_update_time
        lst.append((current_time, total_resources_generated))
    return lst
